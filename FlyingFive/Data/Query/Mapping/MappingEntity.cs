...
namespace FlyingFive.Data.Query.Mapping
{
    public class MappingEntity : IObjectActivatorCreator
    {
        ...
        public IObjectActivator CreateObjectActivator(IDbContext dbContext)
        {
            ...
            {
                IMemberMapper mMapper = mapper.TryGetMappingMemberMapper(kv.Key);
                MappingMemberBinder binder = new MappingMemberBinder(kv.Key, mMapper, kv.Value);
            ...
            Func<IDataReader, DataReaderOrdinalEnumerator, ObjectActivatorEnumerator, object> instanceCreator = this.ConstructorDescriptor.GetInstanceCreator();
